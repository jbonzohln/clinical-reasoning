...
public class ProcessObservationItem {
    ...
    public void process(ProcessParameters processParameters) {
        ...
        final List<IBaseBackboneElement> answers = modelResolverService.getDynamicValues(processParameters.getQuestionnaireResponseItem(), "answer");
        if (!answers.isEmpty()) {
    ...
    private void processAnswer(ProcessParameters processParameters, IBaseBackboneElement answer) {
        final List<IBaseBackboneElement> answerItems = modelResolverService.getDynamicValues(answer, "item");
